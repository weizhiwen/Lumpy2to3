...
class Gui(Tk):
    ...
    explicitly."""
    """Gui类为Tk类中的许多方法提供包装器，
    此外它还可以追踪当前帧画面以便你可以不用显式声明父帧画而创建新的子部件
    """
...
class BBox(list):
    ...
    def copy(self):
        # t = [Pos(coord) for coord in bbox] bbox 列表不存在，这里有误，因改为 list
        t = [Pos(coord) for coord in list]
        return BBox(t)
...
def pair(seq):
    ...
    return [x for x in pairiter(seq)] # 这里使用迭代器在 Python3.7 中会有问题，Python3.6则正常，所以不在使用迭代生成器
