...
# 检查 Python 的大版本号
if v[0] != '3':
    print('你使用Python的大版本应该至少是Python3')
    sys.exit()

# 检查 Python 的次版本号
minor = int(v[1])
if minor > 6:
    print('你使用Python的次版本不能超过Python3.6')
    sys.exit()
...
class Thing(object):
    ...
    # 覆盖 object 父类的 __new__() 方法，注意这里 Python2 和 Python3 的区别
    def __new__(cls, *args, **kwds):
        """override __new__ so we can count the number of Things"""
        Thing.things_created += 1
        return super(Thing,cls).__new__(cls)
...
def make_thing(lumpy, val):
    ...
    simple = (str, bool, int, int, float, complex) # 将 NoneType 类型删除
    # print(type(simple))
